...
class GPTSoVITSPlugin(Star):
    ...
    async def send_voice(self, event: AstrMessageEvent, text: str = ""):
        """这是一个语音输出工具，当用户显式要求你发语音时，你可以调用。
        但你的主要输出仍然是文字，不要用语音代替文字，对这个工具的调用应保持克制。
        或当你觉得此刻非常适合用语音表达时调用。
        例如适合的时机：表达强烈情绪、被要求说话时等。
        语音输出只是点缀，正文仍应以文字回复。
        语音的声线是一位元气可爱的妹妹系，请直接把她当成你的声音。
        Args:
            text(string): 要直接说出的话，请使用元气活泼又动人可爱的妹妹系日语，不要太长；不要进行任何形式的标注，尤其不要用各种括号标注动作或情绪；不要尝试其他语言，最终输出一定会是日语。
        """
        # 防御性检查（配置热改但插件未重载的边界情况）
